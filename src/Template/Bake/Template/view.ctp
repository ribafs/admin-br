...
<div class="actions columns col-lg-2 col-md-3">
    ...
    <ul class="nav nav-stacked nav-pills">
        ...
        <li><?= $this->Form->postLink(__('Delete <%= $singularHumanName %>'), ['action' => 'delete', <%= $pk %>], ['confirm' => __('Deseja excluir realmente # {0}?', <%= $pk %>), 'class' => 'btn-danger']) ?> </li>
...
<div class="related row">
    ...
    <h4 class="subheader"><?= __('Relativo <%= $otherPluralHumanName %>') ?></h4>
    ...
    <% endforeach; %>
            ...
            <tr>
                ...
                <td class="actions">
                    <?= $this->Html->link('<span class="glyphicon glyphicon-zoom-in"></span><span class="sr-only">' . __('Ver') . '</span>', ['controller' => '<%= $details['controller'] %>', 'action' => 'view', <%= $otherPk %>], ['escape' => false, 'class' => 'btn btn-xs btn-default', 'title' => __('Ver')]) ?>
                    <?= $this->Html->link('<span class="glyphicon glyphicon-pencil"></span><span class="sr-only">' . __('Editar') . '</span>', ['controller' => '<%= $details['controller'] %>', 'action' => 'edit', <%= $otherPk %>], ['escape' => false, 'class' => 'btn btn-xs btn-default', 'title' => __('Editar')]) ?>
                    <?= $this->Form->postLink('<span class="glyphicon glyphicon-trash"></span><span class="sr-only">' . __('Excluir') . '</span>', ['controller' => '<%= $details['controller'] %>', 'action' => 'delete', <%= $otherPk %>], ['confirm' => __('Deseja excluir realmente # {0}?', <%= $otherPk %>), 'escape' => false, 'class' => 'btn btn-xs btn-danger', 'title' => __('Excluir')]) ?>
                </td>
            </tr>
            <?php endforeach; ?>
        </table>
